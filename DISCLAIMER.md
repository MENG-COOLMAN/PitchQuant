...
4.6 **第三方数据致谢与责任边界**：本项目的统计结论基于公开数据构建——**Football-Data.co.uk**（赛果/统计/赔率）· **ClubElo**（ELO 评分）· 整合数据集 **xgabora/Club-Football-Match-Data**（Gábor, A.）· **中国体育彩票**官方公开赔率。完整致谢与引用格式见 [NOTICE.md](NOTICE.md) §二。本仓库**不含**上述来源的任何原始数据文件，`tables/` 仅为统计衍生结果。⚠️ **部分联赛官方数据的博彩用途须另行取得权利方许可**（如英/苏职业联赛的 Football DataCo / Genius Sports）——**使用者若将本项目或其衍生结果用于该类用途，须自行取得全部必要许可，与作者无关**。
